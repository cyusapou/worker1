      ...
      <div v-if="currentStep === 0" class="form-section">
        ...
        <div class="form-grid">
          ...
          <div class="form-group form-group--full">
            ...
            <input v-model="form.email" type="email" class="form-input" placeholder="e.g. john@example.com" />
          </div>
        </div>
      </div>
...
const canProceed = computed(() => {
  if (currentStep.value === 0) return form.value.firstName && form.value.lastName && form.value.phone && form.value.email
...
const submitForm = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(errorData.error || 'Failed to register customer')
    }
  } catch (error) {
