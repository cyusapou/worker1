    ...
    <div class="sidebar__user">
      <div class="user-profile">
        <div class="user-avatar overflow-hidden">
          <div v-if="!authStore.user?.avatar" class="user-avatar-initials">{{ userInitials }}</div>
          <img v-else :src="authStore.user.avatar" alt="User" class="avatar-img" />
        </div>
        <div v-if="!isCollapsed" class="user-info">
          <div class="user-name">{{ authStore.user?.name || 'Worker' }}</div>
          <div class="user-role">{{ authStore.user?.category || 'Field Worker' }} • {{ authStore.user?.district || 'Kigali' }}</div>
        </div>
...
const reminderCount = ref(0) // Logic for global reminder counts can be added to pulse later
